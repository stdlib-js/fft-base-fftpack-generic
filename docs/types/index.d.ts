...
import cffti = require( '@stdlib/fft-base-fftpack-generic-cffti' );
...
interface Namespace {
	...
	*/
	...
	/**
	* Initializes a workspace array for performing a cosine transform.
	*
	* ## Notes
	*
	* -   The workspace array should have a length of at least `( 3*N ) + 34` elements.
	...
	* var workspace = new Float64Array( ( 3*N ) + 34 );
	*
	* var out = ns.costi( N, workspace, 1, 0 );
	* // returns <Float64Array>
	*
	* var bool = ( out === workspace );
	* // returns true
	*
	* var cosineTable = workspace.slice( 0, N );
	* // returns <Float64Array>[ 0, ~0.765, ~1.414, ~1.848, 0, ~0.765, ~1.414, ~1.848, 0 ]
	*
	* var twiddleFactors = workspace.slice( (2*N)-1, (3*N)-2 );
	* // returns <Float64Array>[ ~0.707, ~0.707, 0, 0, 0, 0, 0, 0 ]
	*
	* var factors = workspace.slice( (3*N)-2, (3*N)+2 );
	* // returns <Float64Array>[ 8, 2, 2, 4 ]
	*/
	costi: typeof costi;
	...
	* var workspace = new Float64Array( ( 3*N ) + 34 );
	*
	* var out = ns.sinqi( N, workspace, 1, 0 );
	* // returns <Float64Array>
	*
	* var bool = ( out === workspace );
	* // returns true
